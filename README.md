...
Exemplo: kplot "x^2" "x^3" sqrt(x)+3
obs: Alguns caracteres especiais, como o '^', precisam que a expressão esteja envolvida por aspas duplas. São válidos e equivalentes os comandos kplot x*x e kplot x^2

![Funções periódicas](/prints/2.png?raw=true "Função sen(x) + 1")
Exemplo: kplot sin(x)+1
